...
class SWEAgent(BaseAgent):
    def __init__(self, use_fn_calling: bool = False, format_model_response: bool = False, scaffold: str = "r2egym"):
        logger.info(f"Initializing SWEAgent with use_fn_calling: {use_fn_calling}, format_model_response: {format_model_response}, scaffold: {scaffold}")
        ...
        # 根据 scaffold 类型设置系统提示
        if scaffold == "sweagent":
            self.system_prompt = SWEAGENT_SYSTEM_PROMPT
        elif scaffold == "r2egym-debug":
            self.system_prompt = SWE_SYSTEM_PROMPT_DEBUG
        elif scaffold == "r2egym-debug-withtest":
            self.system_prompt = SWE_SYSTEM_PROMPT_DEBUG_WITHTEST
        elif scaffold == "r2egym-debug-enhanced":
            self.system_prompt = SWE_SYSTEM_PROMPT_DEBUG_ENHANCED
        else:  # r2egym (default)
            self.system_prompt = SWE_SYSTEM_PROMPT_FN_CALL if use_fn_calling else SWE_SYSTEM_PROMPT
        
        # 根据 scaffold 类型设置用户提示模板
        if scaffold == "sweagent":
            self.user_prompt_template = SWEAGENT_USER_PROMPT
        elif scaffold in ["r2egym-debug", "r2egym-debug-withtest"]:
            # Debug scaffolds 使用包含 PDB 调试提示的 user prompt
            self.user_prompt_template = SWE_USER_PROMPT_DEBUG
        elif scaffold == "r2egym-debug-enhanced":
            # Enhanced版本：使用更强烈的PDB鼓励提示
            self.user_prompt_template = SWE_USER_PROMPT_DEBUG_ENHANCED
        else:  # r2egym (default)
