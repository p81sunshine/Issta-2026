...
class QwenToolParser(ToolParser):
    ...
    def parse_qwen_tool_calls(self, text: str) -> list[dict[str, Any]]:
        ...
        Returns:
            list[dict]: List of parsed tool calls, each containing 'name' and 'parameters'
            Only parses the last complete tool call in the text.
        """

        tool_calls: list[dict[str, Any]] = []

        # Return empty list if no tool calls found
        if self.tool_call_end not in text:
            return tool_calls

        # Find the last tool_call_end from the end
        end = text.rfind(self.tool_call_end)
        if end == -1:
            return tool_calls

        # Find the nearest tool_call_begin before this end tag
        # Search backwards from the end position
        start_pos = text.rfind(self.tool_call_begin, 0, end)
        if start_pos == -1:
            # No matching begin tag found before the end tag
            print(f"Error parsing tool call: Found </tool_call> but no matching <tool_call> before it")
            return tool_calls

        # Extract the JSON content between the tags
        start = start_pos + len(self.tool_call_begin)
        json_content = text[start:end].strip()

        # Skip empty content
        if not json_content:
            print(f"Error parsing tool call: Empty content between <tool_call> tags")
            print(f"text: {text}")
            return tool_calls
        ...
        try:
            ...
            if not isinstance(call_data, dict):
                print(f"Error parsing tool call: JSON is not a dict: {json_content}")
                print(f"text: {text}")
                return tool_calls
            if "name" not in call_data or "arguments" not in call_data:
                print(f"Error parsing tool call: Missing 'name' or 'arguments' field: {json_content}")
                print(f"text: {text}")
                return tool_calls
            ...
            print(f"Error parsing tool call: Invalid JSON: {json_content}")
            print(f"text: {text}")
            return tool_calls
