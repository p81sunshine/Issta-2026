...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def init_envs_and_agents(self, batch):
        ...
        def _create_env(i):
            ...
            return i, self.env_class.from_dict({**base_env_args, "entry": env_args[i]})
    ...
    def fit_agent(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                # 输出训练阶段开始日志
                colorful_print(f"🚀 [TRAINING PHASE START] Epoch {epoch}, Step {self.global_steps} - Starting training phase", "green")
                ...
                with marked_timer("step", timing_raw):
                    # 输出rollout开始日志
                    colorful_print(f"🎯 [ROLLOUT START] Epoch {epoch}, Step {self.global_steps} - Starting rollout phase", "green")
                    ...
                    # 输出rollout结束日志
                    colorful_print(f"✅ [ROLLOUT END] Epoch {epoch}, Step {self.global_steps} - Rollout phase completed", "green")
                    ...
                    with marked_timer("adv", timing_raw):
                        ...
                        # 输出reward计算结束日志
                        colorful_print(f"✅ [REWARD COMPUTATION END] Epoch {epoch}, Step {self.global_steps} - Reward computation completed", "yellow")
                    ...
                    if self.use_critic:
                        ...
                        # 输出critic更新结束日志
                        colorful_print(f"✅ [CRITIC UPDATE END] Epoch {epoch}, Step {self.global_steps} - Critic update completed", "magenta")

                    # implement critic warmup
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        # 输出actor更新开始日志
                        colorful_print(f" 🎯 [ACTOR UPDATE START] Epoch {epoch}, Step {self.global_steps} - Starting actor update", "green")
                        # update actor
                        ...
                        # 输出actor更新结束日志
                        colorful_print(f"✅ [ACTOR UPDATE END] Epoch {epoch}, Step {self.global_steps} - Actor update completed", "green")
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and self.global_steps % self.config.trainer.test_freq == 0:
                        ...
                        # 输出验证结束日志
                        colorful_print(f"✅ [VALIDATION END] Epoch {epoch}, Step {self.global_steps} - Validation phase completed", "red")
                ...
                # 输出训练阶段结束日志
                colorful_print(f"✅ [TRAINING PHASE END] Epoch {epoch}, Step {self.global_steps} - Training phase completed", "blue")
    ...
    def generate_agent_trajectory(self, timing_raw=None, meta_info=None):
        ...
        with marked_timer("transform_trajectory", timing_raw):
            ...
            final_gen_batch_output, metrics = self._transform_agent_trajectories(trajectories, meta_info=meta_info)
    ...
    def _transform_agent_trajectories(self, trajectories: list[dict], meta_info=None):
        """
        ...
        Args:
            ...
            meta_info (dict, optional): Metadata containing validation flag and other info.
        ...
        # For validation, use validation_data_dir if set; otherwise use rollout_data_dir
        is_validation = meta_info and meta_info.get("validate", False)
        if is_validation:
            val_data_dir = self.config.trainer.get("validation_data_dir", None)
            if val_data_dir:
                save_dir = os.path.join(val_data_dir, "chat_completions")
            else:
                # Fall back to rollout_data_dir if validation_data_dir is not set
                rollout_data_dir = self.config.trainer.get("rollout_data_dir", None)
                if rollout_data_dir:
                    save_dir = os.path.join(rollout_data_dir, "val_chat_completions")
                else:
                    save_dir = os.path.join(self.config.trainer.default_local_dir, "val_chat_completions")
        else:
            # Training rollout: use rollout_data_dir
            rollout_data_dir = self.config.trainer.get("rollout_data_dir", None)
            if rollout_data_dir:
                save_dir = os.path.join(rollout_data_dir, "chat_completions")
            else:
