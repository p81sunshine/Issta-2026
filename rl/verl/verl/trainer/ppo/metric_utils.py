...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Helper functions to handle potentially empty tensors gracefully.
    def _safe_mean(x: torch.Tensor, default: float = 0.0) -> float:
        if x.numel() == 0:
            return default
        return torch.mean(x).detach().item()

    def _safe_max(x: torch.Tensor, default: float = 0.0) -> float:
        if x.numel() == 0:
            return default
        return torch.max(x).detach().item()

    def _safe_min(x: torch.Tensor, default: float = 0.0) -> float:
        if x.numel() == 0:
            return default
        return torch.min(x).detach().item()
    ...
    metrics = {
        # score
        "critic/score/mean": _safe_mean(sequence_score),
        "critic/score/max": _safe_max(sequence_score),
        "critic/score/min": _safe_min(sequence_score),
        # reward
        "critic/rewards/mean": _safe_mean(sequence_reward),
        "critic/rewards/max": _safe_max(sequence_reward),
        "critic/rewards/min": _safe_min(sequence_reward),
        # adv
        "critic/advantages/mean": _safe_mean(valid_adv),
        "critic/advantages/max": _safe_max(valid_adv),
        "critic/advantages/min": _safe_min(valid_adv),
        # returns
        "critic/returns/mean": _safe_mean(valid_returns),
        "critic/returns/max": _safe_max(valid_returns),
        "critic/returns/min": _safe_min(valid_returns),
        **(
            {
                # values
                "critic/values/mean": _safe_mean(valid_values),
                "critic/values/max": _safe_max(valid_values),
                "critic/values/min": _safe_min(valid_values),
                # vf explained var
