...
def test_multi_threaded():
    ...
    assert final_count == expected_total, f"Expected {expected_total} but got {final_count}"


def test_no_active_threads_after_completion():
    counter = ThreadSafeCounter()
    num_threads = 5
    ...
    assert counter.get_active_threads() == 0