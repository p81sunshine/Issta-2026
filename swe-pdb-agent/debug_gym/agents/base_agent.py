...
class BaseAgent:
    ...
    def build_history_prompt(self):
        messages = build_history_prompt(
            ...
            self.config.get("enable_recency_filter", False),
            self.config.get("memory_size", None),
        )
        return messages
    ...
    def save_tool_calls(self, task_name="custom"):
        """Extract and save all tool calls to a JSON file."""
        tool_calls = []
        for step_id in range(len(self.history)):
            step_json = self.history.json(step_id)
            if step_json.get("action") is not None:
                tool_calls.append({
                    "step_id": step_json["step_id"],
                    "tool_call_id": step_json["action"].get("id"),
                    "tool_name": step_json["action"].get("name"),
                    "arguments": step_json["action"].get("arguments"),
                    "response_content": step_json.get("obs", ""),
                })
        
        os.makedirs(pjoin(self._output_path, task_name), exist_ok=True)
        tool_calls_file = pjoin(self._output_path, task_name, "tool_calls.json")
        ...
        self.logger.debug(f"Tool calls saved in {tool_calls_file}")

    def save_conversation(self, task_name="custom"):
        """Save the complete conversation in ChatML format (JSONL)."""
        if self.llm is None:
            self.logger.warning("LLM is None, cannot save conversation in ChatML format")
            return
        
        os.makedirs(pjoin(self._output_path, task_name), exist_ok=True)
        conversation_file = pjoin(self._output_path, task_name, "conversation.jsonl")
        
        with open(conversation_file, "w", encoding="utf-8") as f:
            # Get all history steps
            history_memory = self.history.get_all()
            prompt_response_pairs = self.history.prompt_response_pairs
            
            # Build complete conversation
            conversation_messages = []
            system_prompt_added = False
            
            # Find and add system prompt from the first step that has prompt_response_pairs
            for step_id in range(len(history_memory)):
                if step_id < len(prompt_response_pairs) and prompt_response_pairs[step_id]:
                    step_json = self.history.json(step_id)
                    if step_json.get("prompt_response_pairs") and len(step_json["prompt_response_pairs"]) > 0:
                        first_prompt = step_json["prompt_response_pairs"][0].get("prompt", [])
                        for msg in first_prompt:
                            if msg.get("role") == "system":
                                conversation_messages.append({
                                    "role": "system",
                                    "content": filter_non_utf8(msg.get("content", "")),
                                })
            ...
            # Process each step and format messages
            for step_id in range(len(history_memory)):
                history_info = history_memory[step_id]
                response_list = prompt_response_pairs[step_id] if step_id < len(prompt_response_pairs) else None
                
                # Format messages using LLM's format_tool_call_history method
                if self.llm:
                    formatted_messages = self.llm.format_tool_call_history(history_info, response_list or [])
                    # Add formatted messages to conversation (skip system messages if already added)
                    for msg in formatted_messages:
                        if msg.get("role") == "system" and system_prompt_added:
                            continue
                        conversation_messages.append(msg)
            
            # Write conversation as JSONL (one message per line)
            for msg in conversation_messages:
                f.write(json.dumps(msg, ensure_ascii=False) + "\n")
        
        self.logger.debug(f"Conversation saved in {conversation_file}")
    ...
    def save_trajectory(self, task_name="custom"):
        ...
        # Save additional files: tool_calls.json and conversation.jsonl
        self.save_tool_calls(task_name)
        self.save_conversation(task_name)
