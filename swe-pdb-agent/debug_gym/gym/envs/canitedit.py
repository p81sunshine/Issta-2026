...
class CaniTEditEnv(RepoEnv):
    ...
    def instructions(self) -> str:
        ...
        try:
            ...
            # 不要因为读取说明失败而阻塞任务，直接回退默认提示
            pass

        if question_text:
            return f"{default_instr}\n\nThe task instruction is: \n{question_text}"
        return default_instr
    ...
    def setup_workspace(self):
        ...
        # 仅拷贝除 question.txt 以外的内容，问题描述仍从原始数据目录读取
        with tempfile.TemporaryDirectory() as _tmp:
            tmp_path = Path(_tmp)
            shutil.copytree(
                self.current_task["codebase"],
                tmp_path,
                dirs_exist_ok=True,
                ignore=shutil.ignore_patterns("question.txt"),
            )
            self.workspace.copy_content(src=tmp_path, target=self.workspace.working_dir)
