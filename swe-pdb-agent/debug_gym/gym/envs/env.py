...
class EnvInfo:
    ...
    reset_eval_timeout: bool = False  # Whether eval timed out during reset (should mask trajectory)
    ...
    def __str__(self) -> str:
        ...
        lines.append(f"   {tool_descriptions}")
        lines.append("")
        ...
        if self.current_breakpoints == "No breakpoints are set.":
            ...
            breakpoint_lines = self.current_breakpoints.split("\n")
            for bp in breakpoint_lines[:5]:  # Show first 5
                lines.append(f"   • {bp}")
            if len(breakpoint_lines) > 5:
                remaining = len(breakpoint_lines) - 5
...
class TooledEnv:
    ...
    def get_triggered_tools(self, action: ToolCall):
        try:
            ...
            # Ensure tool_kwargs is a dict, not a string
            # Some LLMs may return arguments as a JSON string instead of a dict
            if isinstance(tool_kwargs, str):
                ...
                try:
                    ...
                    return f"Failed to parse tool arguments as JSON: {tool_kwargs}", None
            if not isinstance(tool_kwargs, dict):
                return f"Tool arguments must be a dict, got {type(tool_kwargs).__name__}", None
        except Exception as e:
            # parse error
            return str(e), None
        # Check for empty tool name to prevent hanging
        if not tool_name or tool_name.strip() == "":
            return (
                "Empty tool name in action. This may indicate a parsing error in the agent's response. "
                "Please ensure your tool call includes a valid tool name.",
                None,
            )
...
class RepoEnv(TooledEnv):
    ...
    def reset(self, *, options: dict = None):
        ...
        # Check if eval timed out during reset (even after retries)
        reset_eval_timeout = False
        if self.last_eval:
            ...
            try:
                ...
                # If parsing fails (e.g., timeout output like "Timeout expired."), use default values
                reset_eval_timeout = True
                self.logger.warning(f"Failed to parse eval output: {e}. Using default values.")
                # Keep existing max_score value (don't reset it)
                # Only reset score, resolved, and terminated
                self.score = 0
                self.resolved = False
                self.terminated = False
        ...
        self.infos = EnvInfo(
            ...
            tools=self.tools,
            reset_eval_timeout=reset_eval_timeout,
        )
        return self.infos
