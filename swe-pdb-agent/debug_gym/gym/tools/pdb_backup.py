...
class PDBTool(EnvironmentTool):
    ...
    examples = [
        ...
        """pdb(command="start", file="tests/test_foo.py") to auto-detect the runner based on the file and project configuration.""",
    ...
    description = (
        ...
        + "\n\n**Starting PDB:**"
        + "\n  Use command='start' with runner and file parameters to start a PDB session:"
        ...
        + "\n  - Auto-detect runner: Just provide file='path/to/file.py' (will detect based on filename and project config)"
        + "\n\n**Other PDB Commands:**"
    ...
    arguments = {
        ...
        "runner": {
            "type": ["string", "null"],
            "description": (
                "The test runner to use. Only applicable with command='start'. Options:\n"
                "  - 'python': Run with 'python -m pdb <file>' (for regular Python scripts)\n"
                "  - 'pytest': Run with 'python -m pytest --pdb <file>' (for pytest tests)\n"
                "If null and file is provided, will auto-detect based on file name/path."
            ),
        },
        "file": {
            "type": ["string", "null"],
    ...
    def _build_entrypoint(
        self, runner: str | None, file: str | None, environment
    ) -> str | None:
        """
        Build the entrypoint command based on runner and file parameters.
        
        Args:
            runner: 'python' or 'pytest' or None
            file: The file path to debug
            environment: The environment instance
            
        Returns:
            The complete entrypoint command, or None if cannot be built
        """
        if not file:
            return None
        ...
        if runner is None:
            ...
            if 'test' in file.lower():
                # Try to detect if project uses pytest
                if (hasattr(environment.workspace, 'file_exists') and 
                    (environment.workspace.file_exists('pytest.ini') or
                     environment.workspace.file_exists('pyproject.toml') or
                     environment.workspace.file_exists('setup.cfg'))):
                    runner = 'pytest'
                else:
                    runner = 'python'
            else:
        ...
        if runner.lower() == 'python':
            ...
            return f"python -m pytest --pdb {file}"
        else:
            return None
    ...
    def use(
        ...
        if command.lower() in ["start", "restart"]:
            ...
            if not file:
                return Observation(
                    ...
                    "  - pdb(command='start', file='tests/test.py')  # auto-detect runner",
                )
            ...
            if new_entrypoint is None:
                return Observation(
                    ...
                    f"Runner must be 'python' or 'pytest', or null for auto-detection.",
                )
