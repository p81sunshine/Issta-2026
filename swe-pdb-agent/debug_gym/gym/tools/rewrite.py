...
class RewriteTool(EnvironmentTool):
    ...
    def use(
        ...
        if not environment.workspace.is_editable(path):
            # Provide more detailed information about why the file is not editable
            readonly_patterns = getattr(environment.workspace, 'readonly_patterns', [])
            if readonly_patterns:
                return self.fail(environment, f"`{path}` is not editable. This file matches one of the readonly patterns: {readonly_patterns}")
            else:
                return self.fail(environment, f"`{path}` is not editable. Check if the file is marked as read-only or is in a protected directory.")
