...
class ViewTool(EnvironmentTool):
    ...
    def use(
        ...
        # 首先解析路径并检测是否为目录；若是目录则返回目录树（ls 风格）结果
        try:
            abs_path = environment.workspace.resolve_path(new_file, raises=True)
            # 不能使用本机的 Path.is_dir()（对 Docker 不可靠），改为在终端内检测
            is_dir_cmd = f'test -d "{abs_path}" && echo DIR || echo NODIR'
            _, dir_check = environment.workspace.terminal.run(is_dir_cmd)
            if dir_check.strip() == "DIR":
                try:
                    result = environment.workspace.directory_tree(root=new_file, max_depth=1)
                except Exception as e:
                    result = f"Error listing directory '{new_file}': {str(e)}"
        ...
        truncated = False
        display_end_index = s + len(selected_lines)
        if len(selected_lines) > 110:
            selected_lines = selected_lines[:110]
            truncated = True
            display_end_index = s + len(selected_lines)
        display_content = "\n".join(selected_lines)
        ...
        line_numbers = (
            f" lines {s + 1}-{display_end_index} of {len(file_lines)} total lines."
            if len(file_lines) > 0
            else ""
        )
        read_only = (
        ...
        truncated_note = (
            f"\n\n... (truncated, showing first 110 lines of the selected range)"
            if truncated
            else ""
        )
        obs = (
