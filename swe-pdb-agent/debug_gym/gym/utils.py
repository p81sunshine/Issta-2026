...
def summarize_pytest_traceback(output):
    """
    Summarize pytest output by removing duplicate patterns in the traceback section.
    
    For the part before "short test summary info":
    - Remove duplicate lines (keep only the first occurrence)
    - Limit to maximum 20 lines
    
    Args:
        output: The pytest output string
        
    Returns:
        The summarized output string
    """
    # Find the position of "short test summary info"
    summary_marker = "short test summary info"
    summary_pos = output.find(summary_marker)
    
    if summary_pos == -1:
        # If summary marker not found, process the entire output
        traceback_part = output
        summary_part = ""
    else:
        # Split into traceback part (before summary) and summary part (from summary onwards)
        traceback_part = output[:summary_pos]
        summary_part = output[summary_pos:]
    
    # Process the traceback part
    ...
    # Limit to maximum 20 lines (keep the last 20 lines)
    if len(unique_lines) > 20:
        unique_lines = unique_lines[-20:]
    
    # Reconstruct the output
    summarized_traceback = "\n".join(unique_lines)
    
    if summary_part:
        return summarized_traceback + "\n" + summary_part
    else:
        return summarized_traceback
