...
class Workspace:
    ...
    def directory_tree(self, root: str | Path = None, max_depth: int = 1):
        ...
        try:
            ...
            self.logger.warning(f"tree command failed: {e}. Using find as fallback.")
            find_cmd = f"find {root} -maxdepth {max_depth} -not -path '*/.*' "
            success, output = self.terminal.run(find_cmd, raises=False)
            if not success or not output:
                return f"{root}/\n  (Unable to list directory - tree command not available)"
            # Convert find output to tree-like format
            lines = [f"{root}/"]
            for line in output.strip().splitlines():
                if line.strip():
                    rel_path = line.replace(str(root), "").strip("/")
                    if rel_path:
                        depth = rel_path.count("/")
                        prefix = "  " * (depth + 1) + "|-- "
                        lines.append(f"{prefix}{os.path.basename(rel_path)}")
            output = "\n".join(lines)
            return output
        ...
        lines = [first]
        for line in rest:
            # Skip lines that don't contain the expected pattern
            if "-- " not in line:
                continue
    ...
    def display_files(self, dir_tree_depth: int = 1) -> str:
        msg = (
            f"Current dir is {self.working_dir}. Listing files in the current working directory."
        )
        msg += self.directory_tree(max_depth=dir_tree_depth)
        return msg + " (read-only) indicates read-only files." + f" Max depth: {str(dir_tree_depth)}.\n"
