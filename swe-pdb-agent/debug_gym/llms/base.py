...
class LLM(ABC):
    ...
    def __init__(
        ...
        # Log model name and effective max context length (in tokens)
        self.logger.debug(
    ...
    def instantiate(
        ...
        elif "DeepSeek-R1-Distill-Qwen-32B" in tags or (
            "vllm" in tags
            and "deepseek" in llm_config.model.lower()
            and "r1" in llm_config.model.lower()
        ):
            from debug_gym.llms import DeepSeekR1LLM
    ...
    def __call__(self, messages, tools, *args, **kwargs) -> LLMResponse:
        ...
        if self.config.ignore_kwargs:
            self.logger.debug(
                "LLM arguments "
                + ", ".join(self.config.ignore_kwargs)
