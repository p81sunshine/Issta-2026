...
from debug_gym.llms.content_parser import extract_tool_call_from_content
...
class OpenAILLM(LLM):
    ...
    def parse_tool_call_response(self, response) -> ToolCall:
        ...
        raw_arguments = getattr(response.function, "arguments", None)
        if raw_arguments in (None, "", "null"):
            ...
            try:
                ...
                self.logger.warning(
                    "无法解析工具调用参数，模型返回的原始内容将被忽略。tool_id=%s tool_name=%s error=%s raw=%r",
                    getattr(response, "id", "unknown"),
                    getattr(response.function, "name", "unknown"),
                    exc,
                    raw_arguments,
                )
                parsed_arguments = {}
    ...
    def generate(self, messages, tools, **kwargs) -> LLMResponse:
        ...
        # Only pass tools if they are provided
        # Some models (e.g., deepseek-r1 on Alibaba Cloud) don't support tool calls
        completion_kwargs = {
        ...
        if tools:
            ...
            # # 检测是否是 R2E-Gym 模型（没有 tool parser，需要在 content 中解析工具调用）
            # model_name_lower = self.config.model.lower()
            # is_r2e_gym = "r2e" in model_name_lower or "r2egym" in model_name_lower
            # if is_r2e_gym:
            #     # R2E-Gym 模型：设置 tool_choice 为 "none"，让模型在 content 中输出 JSON 格式的工具调用
            #     # 这样 VLLM 不会尝试解析工具调用（因为没有 tool parser），避免 400 错误
            #     completion_kwargs["tool_choice"] = "none"
            #     self.logger.debug("R2E-Gym 模型：设置 tool_choice=none，将从 content 中解析工具调用")
            # else:
        ...
        # Parse the tool call from API first
        parsed_tool_call = self.parse_tool_call_response(tool_call)
        
        # If the parsed tool call is empty_tool_response or None, try to extract from content
        # This handles cases where API returns empty_tool_response but content contains valid JSON
        # if (parsed_tool_call.name == "empty_tool_response" or parsed_tool_call is None) and _content and tools:
        #     print(f"✅✅✅ empty!")
        #     extracted_tool_call = extract_tool_call_from_content(_content)
        #     if extracted_tool_call and extracted_tool_call.name != "empty_tool_response":
        #         parsed_tool_call = extracted_tool_call
        #         self.logger.info(
        #             f"从响应内容中提取到工具调用: {extracted_tool_call.name} (API返回的工具调用无效)"
        #         )
