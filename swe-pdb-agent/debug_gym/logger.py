...
class TaskProgress:
    ...
    def log_file_path(self) -> str:
        """Short label shown in the progress table for the log file column.

        Previously this returned the full (possibly very long) path to the
        underlying `debug_gym.log` file, which made the middle column of the
        Rich progress table hard to read.  For the TUI we only need a short
        identifier, so we display the problem id instead.
        """
        return self.problem_id if self.logdir else ""
